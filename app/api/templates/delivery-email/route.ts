...
export async function GET() {
  try {
    ...
    const doc = await db.collection("templates").doc("delivery-email").get();

    if (!doc.exists) {
      return NextResponse.json({ subject: DEFAULT_DELIVERY_SUBJECT });
    }

    return NextResponse.json({
      subject: doc.data()?.subject || DEFAULT_DELIVERY_SUBJECT,
    });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    const db = getAdminFirestore();
    await db.collection("templates").doc("delivery-email").set({ subject });
...
export async function POST(request: Request) {
  try {
    ...
    const { html, text } = renderDeliveryEmail(testVars);
